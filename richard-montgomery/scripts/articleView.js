...
// This function is called in a script tag in new.html to render input on the page.
...
articleView.create = () => {
  ...
  let articleDraft = new Article({
    ...
    publishedOn: $('#checkBox').length > 0 ? new Date() : false
  });

  // TODO: Use our interface to the Handblebars template to put this new article into the DOM:
  let articleHtml = $('#article-template').html();
  let articleTemplate = Handlebars.compile(articleHtml);
  console.log('article html', articleHtml);
  console.log('template', articleTemplate());
  $('#articles').append(articleTemplate(articleDraft));
  ...
  $('#export-field').show();
  var draft = JSON.stringify(articleDraft);
  $('#article-json').val(draft);
};

// COMMENT: Where is this function called? Why?
// If this cooment is in reference to the function below, it is called in a script tag in index.html to call all our functions related to the index page..
